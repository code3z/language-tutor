  ...
  useEffect(() => {
    const updateLessonStatus = async (
      ...
      const { data: lessonsStatusData, error: fetchError } = await supabase
        ...
        .single();
      if (fetchError)
        console.error("Error fetching lesson status:", fetchError);

      const currentStatus = lessonsStatusData?.status
        ? JSON.parse(lessonsStatusData.status)
        : {};
      const newStatus =
        isDone || currentSection === lessonLength - 1 ? "done" : currentSection;
