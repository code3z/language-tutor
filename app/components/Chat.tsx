...
export default function Chat({ language, id, lesson, startItemIndex, startChatIndex, initialChat }: { language: string, id: number, lesson: LessonItem[], startItemIndex: number, startChatIndex: number, initialChat: ChatItem[] }) {
    ...
    const { done } = useLessonStatus(language, id, currentLessonItem, lesson.length);
    ...
    useEffect(() => {
        const updateURL = debounce(() => {
            ...
            if (currentLessonItem === startItemIndex) return; // Don't update URL if we haven't moved from start
            
            const newStartItem = Math.floor(currentLessonItem).toString();
            if (current.get('startItem') !== newStartItem) {
                // Store the current scroll position and form focus
                ...
                router.replace(`/lesson/${language}/${id}${query}`, { scroll: false });
        ...
        updateURL();
    }, [currentLessonItem, router, language, id, searchParams, startItemIndex, chatFormRef]);
        ...
        <div className="bg-background mx-auto px-16 h-screen max-w-3xl py-16 flex flex-col">
            ...
            <div className="flex-grow flex items-center justify-center pb-32 pt-16">
                <div className="w-full flex flex-col" id="chat">
                    {chat.map((item, index) => (
                        <ChatMessage
                            key={('id' in item ? item.id : 'user' in item ? item.user + index : index)}
                            item={item}
                    ...
                    {!done && <form className="mt-8" onSubmit={handleSubmit} ref={chatFormRef}>
                        {!(nextIsChallenge && lesson[currentLessonItem] && !('answer' in lesson[currentLessonItem])) && <input
                            value={input}
                        ...
                        />}
                        <Button disabled={isTyping || (!nextIsChallenge && input === '') || loading} text="Go" small Icon={ArrowRightIcon} />
                    </form>}
                    {!done && <button onClick={dontKnow} className="text-secondary-text text-left mt-2 font-medium"><QuestionMarkCircleIcon className="icon stroke-2" /> I don't know</button>}
